...
RSpec.describe "Users", type: :request do
    ...
    describe "POST /api/v1/auth/signup" do
      context "with valid parameters" do
        it "creates a new user, enqueues an email, and returns JSON data" do
          ...
          user: { name: "shubhi", email: "shubhi2345@gmail.com", password: "password123" } }}.to have_enqueued_job(ActionMailer::MailDeliveryJob)
  ...
  describe "POST /api/v1/auth/login" do
    ...
    it "locks the account out when failed attempts reach 5 or more" do
      ...
      json_response = JSON.parse(response.body)
      expect(json_response['message']).to eq("account locked due to maximum attempts failed please try after sometime")
    end
  end
end
